...
class ServiceImageListener
{
    public function __construct(
        private ImageUploader $uploader,
    ) {}

    public function prePersist(Service $item, $event = null): void
    {
        $this->uploadFile($item, $event);
    }

    public function preUpdate(Service $item, $event = null): void
    {
        $this->uploadFile($item, $event);
    }

    private function uploadFile(Service $item, $event = null): void
    {
        $file = $item->getFile();
        ...
        $safePath = sys_get_temp_dir() . '/service_upload_' . uniqid() . '.webp';
