...
class ServiceGroupImageListener
{
    public function __construct(
        private ImageUploader $uploader,
    ) {}

    public function prePersist(ServiceGroup $item, $event = null): void
    {
        $this->uploadFile($item, $event);
    }

    public function preUpdate(ServiceGroup $item, $event = null): void
    {
        $this->uploadFile($item, $event);
    }

    private function uploadFile(ServiceGroup $item, $event = null): void
    {
        $file = $item->getFile();
        ...
        $safePath = sys_get_temp_dir() . '/service_group_upload_' . uniqid() . '.webp';
